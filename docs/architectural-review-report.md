...
class UserAISession < ApplicationRecord
  belongs_to :user
  acts_as_chat
end

# Использование
session = user.ai_sessions.find_or_create_by(session_type: 'classification')
...
class Digest::Builder
  def initialize(user)
    @user = user
    @session = user.ai_session_for(:digest_generation)
  end
...
class CleanupAISessionsJob < ApplicationJob
  def perform
    AISession.cleanup_old_sessions

    # Сжать большие сессии
    AISession.active.having("COUNT(ai_messages.id) > 100")
      .joins(:ai_messages)
      .group("ai_sessions.id")
...
**Действия:**
1. Создать модель `AISession` с `acts_as_chat`
...
class AddStructuredClassificationToPosts < ActiveRecord::Migration[8.0]
  def change
    ...
    add_foreign_key :posts, :ai_sessions, column: :classified_by_session_id
  end
end
```
...
- [ ] Добавить методы в User модель для управления AI-сессиями
  - [ ] `ai_session_for(type)` - получить или создать сессию
...
- [ ] Реализовать классификацию с использованием AISession
...
- [ ] Учитывать персональный контекст из AISession
...
- [ ] Обновить Feedback модель для интеграции с AISession
- [ ] Добавить `after_create` callback для обновления сессии персонализации
- [ ] Реализовать `add_feedback_example` в AISession
...
- [ ] Использовать AISession для обучения на фидбеке
...
- [ ] Использовать AISession для накопления знаний о пользователе
...
module AI
  class SessionManager
    ...
    def find_or_create_session(type)
      @user.ai_sessions.active.find_or_create_by!(session_type: type) do |s|
